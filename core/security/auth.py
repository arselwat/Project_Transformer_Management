...
# Utilisateur + mot de passe (hash SHA256("password") en exemple)
VALID_USERS = {
    "admin": "5e884898da28047151d0e56f8dc6292773603d0d6aabbdd62a11ef721d1542d8"
    # hash SHA256 du mot de passe "password"
}
...
def login_form() -> None:
    ...
    if st.button("Se connecter", type="primary"):
        ok = user in VALID_USERS and _hash_password(pwd) == VALID_USERS[user]
        if ok:
            st.session_state["auth_ok"] = True
            st.session_state["user"] = user
            st.success("Connecté. Utilisez le menu pour accéder aux pages.")
            st.rerun()  # relance propre
        else:
