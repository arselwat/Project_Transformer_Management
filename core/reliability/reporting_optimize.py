...
def _maintenance_explanation(label: str) -> str:
    text = label.lower()
    if "correct" in text:
        return "Retenue quand les résultats suggèrent des défauts précoces ou une action de correction."
    if "condition" in text:
        return "Retenue quand la surveillance de l’état doit guider la décision d’intervention."
    if "prévent" in text or "prevent" in text:
        return "Retenue quand l’usure devient dominante et qu’un intervalle planifié est pertinent."
...
def _fig_to_rl_image(fig, width_mm=180):
    bio = BytesIO()
...
def _plot_R_curves(fits: Dict[str, Any], intervals: Dict[str, Any]) -> Optional[Any]:
    if not fits:
        return None
    ...
    maybe = []
    for eq, item in (intervals or {}).items():
...
def _summary_table_data(
    ...
    for eq in eqs:
        ...
        data.append([
            ...
            _san(row.get("thermal_status", "—")) if row else "—",
        ])
...
def _parameter_influence_table(row: Dict[str, Any], pipe: Dict[str, Any]) -> List[List[Any]]:
    ...
    thermal_status = _san(row.get("thermal_status", "—"))
    maintenance = _maintenance_type(beta, row.get("maintenance_type"))
    faa = _safe_float(row.get("FAA_max"))
    loss = _safe_float(row.get("loss_of_life_pct"))
    ...
    return [
        ...
        ["Processus retenu", model, "Le type de processus influence la prudence du plan proposé."],
        ...
        ["Statut thermique", thermal_status, "Peut accélérer la décision si une alerte ou un état critique existe."],
        ["FAA maximal", _fmt(faa, 3), "Mesure l’intensité du vieillissement thermique."],
        ["Perte de vie (%)", _fmt(loss, 3), "Mesure la part estimée de vie déjà consommée."],
    ]
...
def export_optimization_report_pdf_bytes(
    *,
    ...
    story.append(Paragraph(_san(f"- Équipements analysés : {nb_eq}"), styles["BodyText"]))
    story.append(Paragraph(_san(f"- Observations TTF : {nb_obs}"), styles["BodyText"]))
    story.append(Spacer(1, 8))

    if meta:
        story.append(Paragraph("Paramètres utilisés", styles["Heading2"]))
        meta_rows = [["Paramètre", "Valeur"]]
        for key in ["alpha", "R_target", "C_prev", "C_corr", "R_min_cost", "faa_limit", "lol_limit_pct"]:
            if key in meta:
    ...
    story.append(Paragraph(
        "Le rapport présente uniquement les éléments utiles à l’optimisation : paramètres Weibull, intervalles calculés, influence thermique et type de maintenance retenu.",
        styles["BodyText"],
    ))
    story.append(Spacer(1, 10))
    ...
    for idx, eq in enumerate(eqs):
        ...
        story.append(Paragraph(_compact(_pipe_line(pipe), 220), styles["BodyText"]))
        ...
        story.append(Paragraph(
            _san(
                ...
                f"Ce choix résulte de la combinaison entre la fiabilité, l’intervalle optimisé et l’état thermique."
            ),
            styles["BodyText"],
        ))
        story.append(Spacer(1, 5))
        ...
        if hasattr(thermal_summary, "empty") and not thermal_summary.empty:
            story.append(Paragraph("Synthèse thermique", styles["Heading3"]))
            tdata = [list(thermal_summary.columns)] + thermal_summary.astype(str).values.tolist()
            story.append(_mk_table(tdata, total_width=usable_width, font_size=7))
            story.append(Spacer(1, 6))
