...
with page_tabs[4]:
    ...
    if len(results_by_equipment) > 1:
        st.info(
            "Plusieurs transformateurs sont sélectionnés : les courbes sont superposées sur les mêmes axes, "
            "avec des couleurs différentes pour chaque équipement."
        )
    ...
    for curve_tab, curve_definition in zip(reliability_curve_tabs, curve_definitions):
        ...
        with curve_tab:
            ...
            if len(equipment_codes_for_plots) == 1:
                st.caption(
                ...
                st.caption(
                    "Lecture comparative : chaque courbe correspond à un transformateur différent. "
                    "La comparaison visuelle permet d'identifier les écarts de survie, de densité et d'intensité."
                )
