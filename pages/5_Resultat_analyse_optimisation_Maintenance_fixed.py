...
with st.expander("Comprendre clairement les tableaux affichés sur cette page", expanded=False):
    st.markdown(
        """
Cette page présente les résultats sous forme de **tableaux proches de ceux d’un mémoire** tout en gardant les **graphiques des méthodes graphiques**.

Vous y trouvez :
- un tableau global de **validation des tests de tendance** ;
- un tableau global de **validation des tests de dépendance** ;
- un tableau de **loi, estimation des paramètres et temps de remplacement** ;
- un tableau de **décision finale hiérarchisée** ;
- puis, pour chaque équipement, les **graphiques** et **tableaux détaillés** du pipeline.

**Test d’hypothèse** : chaque test compare une hypothèse nulle, par exemple l’absence de tendance ou l’absence de dépendance, à une hypothèse alternative.
Si la statistique sort de la zone critique ou si la valeur p est inférieure au seuil alpha, l’hypothèse nulle est rejetée.

**Estimation des paramètres** : les paramètres comme **beta**, **eta** et **gamma** sont calculés à partir de la loi retenue.
- **beta** décrit la forme du vieillissement ;
- **eta** représente une durée de vie caractéristique ;
- **gamma** représente un décalage temporel éventuel.

Les **jours avant maintenance** viennent de l’intervalle de maintenance retenu, converti en jours.
Ils ne remplacent pas le **MTBF** :
- le **MTBF** décrit un comportement moyen entre défaillances ;
- les **jours avant maintenance** correspondent à une échéance opérationnelle de planification.
        """
    )
...
with st.spinner("Analyse globale en cours..."):
    ...
    for equipment_code in results_by_equipment.keys():
        ...
        detail_tables_by_equipment[equipment_code] = build_detail_excel_tables(selected_result, selected_row, alpha_value)
        detail_tables_by_equipment[equipment_code]["__payload__"] = {
            "ttf_series": time_to_failure_series,
            "alpha": alpha_value,
            "reliability": selected_result.get("reliability", {}) or {},
        }
...
with page_tab_2:
    ...
    payload = selected_tables.get("__payload__", {}) or {}
    reliability_result = payload.get("reliability", selected_result.get("reliability", {}) or {})
    ...
    st.markdown("### 4. Ajustement et lois candidates")
    ...
    st.markdown("### 5. Paramètres fiabilistes")
    ...
    st.markdown("### 7. Optimisation et maintenance")
