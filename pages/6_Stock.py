...
with tab_list:
    ...
    for sp in SPARE_PARTS:
        code = sp.get("code")
        if not code:
            # on ignore les lignes mal formées de SPARE_PARTS
            continue
        code = str(code)
    ...
    st.caption(
        "Saisis uniquement les **quantités à ajouter** et ajuste les **seuils minimums** si nécessaire."
    )

    with st.form("maj_spares"):
        qty_inputs: dict[str, int] = {}
        seuil_inputs: dict[str, int] = {}
    ...
    st.caption(
        "Pour ajouter d’autres références hors liste recommandée, utilise ce formulaire."
    )
    ...
    with colB:
        st.caption(
            "Purge / suppression définitive à gérer directement dans le CSV si besoin."
        )
    ...
    if auto and low:
        ...
        if _t.time() - last > 600:
            try:
                ...
                # en cas d'erreur d’alerte automatique, on ne casse pas la page
                pass
