...
render_shell("pages/6_Stock.py")
render_page_header(
    "Stock",
    "Créer les articles, gérer les entrées et sorties, et surveiller les seuils.",
    "📦",
)
...
with tab_stock:
    ...
    with copt2:
        st.caption("Actif = stock > 0, seuil > 0, ou sous seuil.")
    ...
    low = _low_stock_list()
    if low:
        st.warning(f"{len(low)} article(s) sous le seuil.")
    else:
        st.success("Aucun article sous seuil.")

    ok_auto, _ = _send_low_stock_alert(low, force=False)
    if ok_auto:
            ...
            with st.container(border=True):
                ...
                with c3:
                    ...
                    with a:
                        ...
                        if st.button("Valider entrée", key=f"btn_add_{code}", use_container_width=True):
                            if qty_add <= 0:
                    ...
                    with b:
                        ...
                        if st.button("Valider sortie", key=f"btn_out_{code}", use_container_width=True):
                            if qty_out <= 0:
                                st.info("Saisis une quantité > 0.")
                            else:
    ...
    st.divider()
    st.subheader("Envoi manuel des alertes")

    cman1, cman2 = st.columns([1.2, 2.0])
    with cman1:
        force_send = st.toggle("Forcer l’envoi", value=False)
    with cman2:
        st.caption("Ignore le cooldown de 10 minutes si nécessaire.")
