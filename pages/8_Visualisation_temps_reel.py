...
render_shell("pages/8_Visualisation_temps_reel.py")
render_page_header(
    "Temps réel",
    "Simulation et visualisation en temps réel des transformateurs.",
    "📡",
)
...
if "rt_data_map" not in st.session_state:
    st.session_state.rt_data_map = {}
if "events_map" not in st.session_state:
    st.session_state.events_map = {}
if "running_map" not in st.session_state:
    st.session_state.running_map = {}
...
def _append_events_to_csv(events: list[dict]) -> None:
    ...
    header = ["ts", "level", "code", "msg", "value", "threshold", "site", "equipment", "equipment_code"]
...
if not rows:
    st.warning("Aucun transformateur actif. Va d’abord dans la page Transformateurs.")
    st.stop()

codes = [r["equipment_code"] for r in rows if r.get("equipment_code")]
pre = str(st.query_params.get("tfm", "")) if st.query_params else ""
default_index = codes.index(pre) if pre and pre in codes else 0

mode_multi = st.toggle("Mode multi-transformateurs (max 3)", value=False, key="rt_multi")
...
if not mode_multi:
    ...
    site_name = sel_rec.get("site") or ""
    equip_name = sel_rec.get("name") or code
    equip_code = code
    ...
    with st.container(border=True):
        ...
        with c1:
            st.caption("Nom / Modèle")
            st.write(sel_rec.get("name") or "—")
            st.caption("Site")
            st.write(sel_rec.get("site") or "—")
            st.caption("Mise en service")
            st.write(sel_rec.get("commissioned_on") or "—")
        with c2:
            st.caption("Puissance (MVA)")
            st.write(f"{float(sel_rec.get('rated_mva', 0) or 0):.2f}")
            st.caption("V1n (kV)")
            st.write(f"{float(sel_rec.get('V1n_kV', 0) or 0):.3f}")
            st.caption("V2n (kV)")
            st.write(f"{float(sel_rec.get('V2n_kV', 0) or 0):.3f}")
        with c3:
            st.caption("Fréquence (Hz)")
            st.write(f"{float(sel_rec.get('f_nominal', 0) or 0):.0f}")
            st.caption("Groupe vectoriel")
            st.write(sel_rec.get("vector_group") or "—")
            st.caption("Code")
            st.write(code)
    ...
    def _tick_one():
        ...
        base_load = np.clip(float(load_pct) * (1.0 + 0.03 * np.random.randn()), 0.0, 200.0)
        base_pf = np.clip(float(pf_prim) + 0.01 * np.random.randn(), 0.5, 1.0)
        base_Tamb = np.clip(float(Tamb) + 1.0 * np.random.randn(), 0.0, 60.0)
        base_freq = np.clip(float(freq_hz) + 0.02 * np.random.randn(), 49.0, 51.0)
        ...
        if (now_m - st.session_state.last_tick) >= target_dt * 0.95:
            ...
            if E:
                ...
                for e in E:
                    e2 = dict(e)
                    e2.setdefault("equipment", equip_name)
                    e2.setdefault("equipment_code", equip_code)
                    e2.setdefault("site", site_name)
                    enriched.append(e2)

                evbuf.extend(enriched)
                st.session_state.events_map[code] = evbuf[-1000:]

                _append_events_to_csv(enriched)
                for e in enriched:
                    try:
                        notify_event(e)
                    except Exception:
                ...
                for e in enriched:
                    if e.get("level") == "ALARM":
                        st.error(f"{e.get('code')}: {e.get('msg')}")
                    elif e.get("level") == "WARN":
                        st.warning(f"{e.get('code')}: {e.get('msg')}")
    ...
    with m1:
        st.metric("V sec (kV)", _fmt_scaled(last.get("v_sec"), 1e3, "{:.3f}"))
    with m2:
        st.metric("I sec (kA)", _fmt_scaled(last.get("i_sec"), 1e3, "{:.3f}"))
    with m3:
        st.metric("P nette (MW)", _fmt_scaled(last.get("p_net"), 1e6, "{:.3f}"))
    with m4:
        st.metric("Hot-spot (°C)", _fmt(last.get("t_core"), "{:.1f}"))
    with m5:
        st.metric("PF prim", _fmt(last.get("pf_prim"), "{:.3f}"))
    with m6:
    ...
    with tabs[0]:
        ...
        if ycols:
            st.plotly_chart(px.line(df_ht, x="t", y=ycols, title="Secondaire — tension & courant"), use_container_width=True)

    with tabs[1]:
        ycols = [c for c in ["p_sec_MW", "p_net_MW", "p_loss_MW"] if c in df_ht.columns]
        if ycols:
            st.plotly_chart(px.line(df_ht, x="t", y=ycols, title="Puissance"), use_container_width=True)

    with tabs[2]:
        ycols = [c for c in ["v_prim_kV", "i_prim_kA"] if c in df_ht.columns]
        if ycols:
            st.plotly_chart(px.line(df_ht, x="t", y=ycols, title="Primaire"), use_container_width=True)
    ...
    with tabs[4]:
        ...
        if ycols:
            st.plotly_chart(px.line(df_view, x="t", y=ycols, title="PF & fréquence"), use_container_width=True)
    ...
    with tabs[5]:
        ...
        TEMP_WARN = p.TEMP_WARN
        TEMP_ALARM = p.TEMP_ALARM
        ...
        if "load_pct" in dfA.columns or "eta" in dfA.columns:
            ...
            st.plotly_chart(px.line(dfA, x="t", y=ycols, title="Charge (%) & rendement (%)"), use_container_width=True)
    ...
    with tabs[6]:
        ...
        if not ev.empty:
            ...
            cols = [c for c in ["t", "level", "code", "msg", "value", "threshold", "site", "equipment", "equipment_code"] if c in ev.columns]
        ...
        if (now_m - st.session_state.last_tick) >= target_dt * 0.95:
            for code in sel_codes:
                ...
                if E:
                    evbuf = st.session_state.events_map[code]
                    enriched = []
                    meta = tfm_meta.get(code, {})
                    for e in E:
                        e2 = dict(e)
                        e2.setdefault("equipment", meta.get("equipment", code))
                        e2.setdefault("equipment_code", meta.get("equipment_code", code))
                        e2.setdefault("site", meta.get("site", ""))
                        enriched.append(e2)
