...
from core.notify.config import load_alerts_config, save_alerts_config
...
render_shell("pages/9_Parametres_Alertes.py")
render_page_header(
    "Paramètres alertes",
    "Configurer les alertes e-mail et WhatsApp pour les événements temps réel.",
    "🔔",
)
...
with colG1:
    cfg.cooldown_minutes = st.number_input(
        "Cooldown e-mail (minutes)",
        min_value=1,
        step=1,
        value=int(cfg.cooldown_minutes or 15),
    )
with colG2:
    cfg.min_period_s = st.number_input(
        "Période minimale legacy (s)",
        min_value=0.0,
        step=5.0,
        value=float(cfg.min_period_s or 30.0),
    )

levels_all = ["INFO", "WARN", "ALARM"]
cfg.levels = st.multiselect(
    "Niveaux qui déclenchent l’e-mail",
    options=levels_all,
    default=cfg.levels or ["WARN", "ALARM"],
)

sites_txt = st.text_input(
    "Filtre sites (séparés par virgules, vide = tous)",
    value=",".join(cfg.site_filter or []),
)
...
with c2:
    ...
    cfg.smtp.to_addrs = [
        s.strip() for s in st.text_input("TO (séparés par virgules)", value=",".join(cfg.smtp.to_addrs or [])).split(",") if s.strip()
    ]
...
with c5:
    ...
    cfg.twilio.whatsapp_from = st.text_input("WHATSAPP_FROM", value=cfg.twilio.whatsapp_from or "")
with c6:
    cfg.twilio.token = st.text_input("TWILIO_TOKEN", type="password", value=cfg.twilio.token or "")
    cfg.twilio.whatsapp_to = st.text_input("WHATSAPP_TO", value=cfg.twilio.whatsapp_to or "")
...
st.caption("Paramètres enregistrés dans le dossier data de l’application.")